...
exports.run = (client, msg, args) => {
	...
	if (!user) return msg.reply('Du musst einen User angeben, welchem du Coins wegnehmen willst.');
	...
	sql.get(`SELECT * FROM scores WHERE userId ="${user.id}"`).then(row => {
		...
		msg.reply("Punkte wurden erfolgreich geupdatet!").then(m => m.delete({ timeout: 5000 }));
	  }).catch((error) => {
		console.error(error);
...
exports.help = {
	name: 'removepoints',
	description: 'Zieht einem User Punkte ab',
	usage: 'removepoints @USER {ANZAHL}',
	example: 'removepoints @Fabiiaan 30',
	category: 'botowner'
};
